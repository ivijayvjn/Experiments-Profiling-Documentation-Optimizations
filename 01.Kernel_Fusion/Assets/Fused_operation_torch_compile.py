...
def fused_operation(x):
    return torch.sigmoid(torch.relu(x + 1.0) * 2.0)
fused_operation = torch.compile(fused_operation , mode = "reduce-overhead")    
    #Experiment
def run_bench():
...
#starting the 100 iterations here 
    ...
    print(f"Fused Average latency : {avg_latency: .4f} ms")
